...
| Update post | Yes (PATCH with `updateMask`); text and other fields only, media cannot be changed |
| Delete post | Yes |
...
|---|---|---|
| Text only | No `media` | Publishes a plain text local post |
| Single image | One image entry in `media` | GBP supports one image per post; additional entries are ignored |
...
**Carousels and multi-image posts:** Not supported. Only the first entry in `media` is used.
...
An update that carries media is rejected with `validation.media_not_updatable`. Google does not document whether the local posts PATCH endpoint accepts a `media` update mask, so SocialAPI returns a clear error rather than accepting the request and silently discarding the media. To change a post's image, delete the post and create a new one.
...
If the post no longer exists on GBP, the call returns `404` with code `resource.not_found`.
...
Google OAuth 2.0 access tokens are short-lived. SocialAPI keeps the connection valid automatically. No action is required from your application.

### Rate limits

Google Business Profile enforces a per-minute quota on API traffic. When it is reached, requests return `429 Too Many Requests`. Retry after a short delay.
...
| Scope | Purpose |
|---|---|
| `https://www.googleapis.com/auth/business.manage` | Read and write access to all Google Business Profile locations, posts, and reviews for the authenticated user |
