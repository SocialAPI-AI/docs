...
| Multi-photo album | 2 to 10 image URLs in `media_ids` | Each photo is uploaded with `published=false`, then a single `/{page-id}/feed` call attaches all `media_fbid` values via `attached_media[]`. `platform_data.facebook` fields (`link`, CTA, age targeting) are forwarded to the feed call. |
...
Unlike single-photo posts, `platform_data.facebook` IS honored for albums. The `link`, `cta_type` plus `cta_link`, and age targeting fields are forwarded to the feed call:
