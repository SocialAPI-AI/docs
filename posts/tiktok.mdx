...
6. On success, the published item ID is stored and returned. TikTok populates that ID (`publicaly_available_post_id`) asynchronously and it can lag `PUBLISH_COMPLETE` by a few seconds, so SocialAPI keeps the target at `publishing` until the ID is in hand. See [Slow publishes and the publishing status](#slow-publishes-and-the-publishing-status).
...
SocialAPI polls the publish status for up to 60 seconds. TikTok often needs longer, either because the video is still being processed or because it has finished but has not yet exposed the post ID. In both cases the target stays at `status: "publishing"` with `platform_post_id` and `permalink` absent, and SocialAPI keeps TikTok's publish handle so it can resolve the outcome later. A background pass re-queries TikTok every 15 minutes and writes the real result, so the target settles at `published` with its ID and permalink, or at `failed` if TikTok rejected it.
...
- **Do not retry or republish it.** The upload may already be live on TikTok, so retrying risks a duplicate post. The reconcile pass is what resolves it.
- **Keep polling `GET /v1/posts/{post_id}`, or wait for the webhook.** The `post.published`, `post.partial` and `post.failed` webhooks fire when the post reaches its terminal status, including when that happens during reconciliation.
