...
Use `POST /v1/posts` with `account_ids` targeting a TikTok account. The `text` field becomes the post title or description. `privacy_level` is required in `platform_data.tiktok`.
...
|---|---|---|---|
| `privacy_level` | string | Yes | Audience setting. Must be one of the values returned by the creator info endpoint for this account. Common values: `PUBLIC_TO_EVERYONE`, `MUTUAL_FOLLOW_FRIENDS`, `FOLLOWER_OF_CREATOR`, `SELF_ONLY`. |
...
### privacy\_level is required and per-creator
