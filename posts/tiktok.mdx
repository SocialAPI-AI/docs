...
Before publishing, the privacy level, posting availability, and creator-level comment, duet, and stitch settings are validated against the creator's current TikTok settings.
...
Privacy level options differ per creator. Always call `POST /v1/posts/validate` before publishing to confirm the target privacy level is allowed for the specific account. Creator info reflects the creator's current TikTok settings.
...
1. The creator's current TikTok settings and allowed privacy levels are read.
2. The draft is validated (privacy level required, duet/stitch for video, brand content rules).
3. TikTok pulls the media directly from the URLs supplied.
4. The target settles once TikTok reports the post complete or failed.
5. TikTok populates the published post ID asynchronously and it can lag completion by a few seconds, so the target stays at `publishing` until the ID is in hand. See [Slow publishes and the publishing status](#slow-publishes-and-the-publishing-status).
...
`GET /v1/posts` returns the TikTok posts SocialAPI knows about. Posts created natively on TikTok are imported automatically, which requires the `video.list` scope: that scope is subject to TikTok app review and may not be active for all connected accounts.

**What this means in practice:**

- Posts created through SocialAPI are returned by `GET /v1/posts` as soon as they publish.
- TikTok-native posts (created outside SocialAPI) appear once the `video.list` scope is approved for the app.
- Engagement metrics (likes, comments, shares, views) are refreshed alongside them.
...
When a creator has reached their daily TikTok posting limit, TikTok reports an empty `privacy_level_options` list, which surfaces as a `400` validation error with field `posting_limit`.

### Token auto-refresh

TikTok access tokens expire. SocialAPI refreshes them automatically. If the refresh token has also expired, calls return `401 invalid_token` and the account must be reconnected.

### Slow publishes and the `publishing` status

TikTok often takes longer to settle than the publish request does, either because the video is still being processed or because it has finished but has not yet exposed the post ID. In both cases the target stays at `status: "publishing"` with `platform_post_id` and `permalink` absent, and the outcome is resolved automatically afterwards: the target settles at `published` with its ID and permalink, or at `failed` if TikTok rejected it.

`publishing` is not terminal, so treat it as "still working" rather than success or failure:

- **Do not confirm success on it.** Only `published` is a completed publish, and a `published` TikTok target always carries a `platform_post_id`.
- **Do not retry or republish it.** The upload may already be live on TikTok, so retrying risks a duplicate post. It resolves on its own.
- **Keep polling `GET /v1/posts/{post_id}`, or wait for the webhook.** The `post.published`, `post.partial` and `post.failed` webhooks fire when the post reaches its terminal status, including when it happens after the original request returned.

Typical settle time is a few seconds to a few minutes. A target that TikTok never resolves is failed after about 2 hours with error code `system.internal` and the detail `publish timed out: target stuck in publishing`. Check the TikTok profile before retrying such a target: the post can still have gone live.
