...
Use `POST /v1/posts` with `targets` targeting a LinkedIn organization page. The `text` field becomes the `commentary` field in the LinkedIn post body.
...
| `scheduled_at` | Deferred publish | ISO 8601 UTC timestamp |
...
| Scope | Purpose |
|---|---|
| `r_liteprofile` | Read the authenticated user's basic profile |
...
| `r_organization_social_feed` | Read organization social feeds (comments, reactions) |
| `w_organization_social_feed` | Reply to and moderate organization social feeds |
| `rw_organization_admin` | List and manage organization page admin roles (required to enumerate connected pages) |

All six scopes are requested. If `w_organization_social` is missing, post creation and deletion fail with a `403 Forbidden` from the LinkedIn API. If `rw_organization_admin` is missing, account connection fails because SocialAPI cannot enumerate the organization pages the user administers.
