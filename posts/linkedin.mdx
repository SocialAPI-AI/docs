...
`connections_only` is a member-only value. Organization page posts (`linkedin_page` platform) do not support `CONNECTIONS` visibility; the LinkedIn API will reject it. Use `public` or `logged_in` for org page posts.
