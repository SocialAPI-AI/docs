...
Threads publishing behaves differently by content type. Text posts publish in one step. Image and carousel posts publish as soon as Threads accepts the media. Video posts publish once Threads has finished processing the file, which adds a delay.
...
|---|---|---|
| Text only | Omit `media` | Supported natively; unlike Instagram, text-only posts work without any media |
| Single image | One `media` entry pointing to an image | Sent as `media_type: IMAGE` |
| Single video | One `media` entry pointing to a video (`.mp4`, `.mov`, `.avi`, `.mkv`, `.webm`) | Sent as `media_type: VIDEO`; container is polled until `FINISHED` |
| Carousel | Two or more `media` entries | Up to 20 items; mix of images and videos allowed |

SocialAPI auto-detects the media type from the URL file extension. The extension is read from the URL path, so query parameters (for example on a presigned upload URL) do not affect detection.
...
Threads processes video before it can be published. The post goes live as soon as processing finishes. If Threads rejects or expires the media, the publish fails. If processing does not finish within 5 minutes, the publish fails with `504 Gateway Timeout` and the post target is marked `failed`.

Image and text posts publish without this delay.
...
Unlike Instagram, Threads supports text-only posts natively. Omit `media` entirely to create a post with just a text body. The container is created with `media_type: TEXT`.
...
Threads long-lived user access tokens expire after approximately 60 days. When a token expires, publish calls will fail with an `auth` error. Reconnect the account via the OAuth flow to obtain a fresh token.
...
```json
{
  "upload_url": "https://upload.example.com/media/clip.mp4?X-Amz-Signature=...",
...
Response (publishing continues in the background):
