...
Article creation on Zalo is asynchronous: Zalo does not return a finished article straight away. The target settles once Zalo publishes it and a final article ID is available. If the article is still processing by the time the request gives up, the target lands in a failed state carrying `platform.zalo.article_processing`. The article may still finish publishing on Zalo's side, so check the Official Account before retrying, otherwise you risk publishing a duplicate.

### Draft validation

Drafts are validated before the article is created. Zalo requires a title: if both the title and text are empty, the draft is rejected with:
