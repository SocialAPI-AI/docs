...
Partial edits are safe on both endpoints: fields you do not provide are preserved. SocialAPI fetches the current video and backfills any unspecified snippet field, so an omitted field is never wiped.
...
- Omit `tags` to leave the video's existing tags unchanged. Sending an empty array is a no-op rather than a clear, since the current tags are backfilled. Clearing tags is not supported through the API.
