---
title: Claude
description: Connect SocialAPI.AI to Claude on web, desktop, CLI, and mobile.
---
...
### 2. Connect SocialAPI.AI

Browse the connectors directory, search for **SocialAPI.AI**, and click **Connect**. There is nothing to type in: Claude already knows the server URL.
...
You can still add the server by URL, which is useful if your organization restricts the directory. Scroll to the bottom of the Connectors page and click **Add custom connector**.
...
Both routes reach the same server and expose the same tools.
...
3. Search the directory for **SocialAPI.AI** and click **Connect**. To add it by URL instead, click **Add custom connector** and enter `SocialAPI.ai` as the name and `https://api.social-api.ai/mcp` as the URL
