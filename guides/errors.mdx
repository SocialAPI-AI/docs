...
| `publishing.delivery.*` | Publishing to a platform failed mid-delivery |
...
| `http.<status>` | Transport-level HTTP error (for example, `http.404` for an unknown route). Not returned by application logic. |
...
| 403 | `platform.youtube.account_suspended` | YouTube has suspended or closed the account, or its channel. The token is still valid, so reconnecting does not restore access: the suspension has to be resolved with YouTube directly. SocialAPI stops background sync for the account and sets `reconnect_reason` to `youtube_account_suspended` |
...
These arise during post delivery and surface in the inbox / events stream rather than as direct HTTP responses, but they appear in `event.error_code` for `post.failed` and `post.partial` events.
...
| 202 | `platform.zalo.article_processing` | Article creation is still processing on Zalo when the request gave up waiting; the article may still publish, so check before retrying |
