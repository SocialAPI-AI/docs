...
Use `code` for programmatic handling; use `error` for logging and display. Codes are stable and namespaced with dot-separated segments, so you can match on a prefix (for example `platform.*` for any upstream platform failure, or `byok.*` for any bring-your-own-key issue).
...
The `code` always starts with `validation.`. The specific sub-code after the dot identifies the field or rule that failed; `meta.field` in the response points to the offending field name when applicable.

## Resources

| HTTP | Code | When it occurs |
|---|---|---|
| 404 | `resource.not_found` | Generic resource lookup miss; check the `meta.resource` and `meta.id` fields |
...
| 403 | `billing.post_limit` | Monthly post allowance is exhausted |
...
Limit responses include `meta.plan` and `meta.max` so clients can prompt the user to upgrade.
...
```python
if error["code"] == "account.reconnection_required" or \
   error["code"].endswith(".auth") and error["code"].startswith("platform."):
...
`billing.post_limit` (HTTP 403) means your monthly allowance is exhausted. Limits reset at the start of your next billing period. The free plan uses a rolling 30-day window from your account anniversary.
...
Any `validation.*` code includes a `meta.field` pointing to the offending input. Surface the field name to the user.
...
if (!resp.ok) {
  ...
  if (code.startsWith("billing.")) {
    throw new Error(`Plan limit hit: ${err.error}`);
  }

  // BYOK setup needed
  if (code === "byok.credentials_missing") {
    showByokSetupModal(err.meta.platform);
    return;
  }

  // Operation not supported on this platform
  if (code === "resource.not_supported") {
    console.warn(`Not supported: ${err.error}`);
    return null;
  }

  throw new Error(`API error: ${err.error} (${code})`);
}
```
