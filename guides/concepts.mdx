...
SocialAPI.ai is a unified social media API: one REST endpoint and MCP server to read and respond to comments, direct messages, mentions, and reviews across Instagram, Facebook, TikTok, LinkedIn, YouTube, X, and Threads. Every platform returns the same `Interaction` schema.
...
An **Interaction** is anything inbound: a comment on one of your posts, a DM, a review, or a mention. The unified inbox API never stores interaction *content* (always proxied live from the platform), but it does mint stable IDs so your app can reference them.
