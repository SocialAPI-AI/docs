...
| `account.connected` | A social account finished connecting. Payload includes username, display name, profile picture, bio, and a public profile link. |
| `account.disconnected` | A connected account was revoked by the user on the platform side (Meta deauthorize webhook, TikTok deauthorize webhook). |

You can subscribe to any combination. Pass all eight to receive everything:
