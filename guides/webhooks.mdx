...
| `review.reply_state_changed` | inbox | A reply you posted to a review finished moderation (Google Business Profile). The payload carries `account_id`, `review_id`, and `state`, which is `APPROVED` (the reply is now public) or `REJECTED` (it will never publish). Read the review to see `reply.policy_violation` for the reason |
