...
The middleware emits a consistent set of error codes when a request cannot be authenticated. Use `error.code` for programmatic handling.
...
| 401 | `auth.invalid_key` | API key prefix detected but key does not match any active key |
| 401 | `auth.invalid_token` | Bearer JWT or OAuth access token is invalid or expired |
| 401 | `auth.not_provisioned` | Dashboard JWT resolved a user that has not been provisioned; call `POST /v1/users/provision` first |
| 401 | `auth.user_not_found` | Token resolves to a user that no longer exists |
| 403 | `auth.dashboard_only` | Endpoint only accepts dashboard JWTs, not raw API keys |
