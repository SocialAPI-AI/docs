...
`daily_limit` is `-1` for Enterprise (unlimited). Upgrade at [app.social-api.ai](https://app.social-api.ai) → **Billing**.
...
<Note>
  If the rate limiter backend is unavailable, the API **fails open** — requests are allowed through rather than blocked. This prevents infrastructure downtime from taking down your integration.
</Note>
