...
{
  "error": {
    "code": "billing.post_limit",
    "message": "not enough post credits: need 1, have 0 remaining"
  }
}
```

The interaction-limit response uses the same shape with `error.code` set to `billing.interaction_limit`. Your counters reset at the start of your next billing period. To get more capacity immediately, upgrade your plan.
