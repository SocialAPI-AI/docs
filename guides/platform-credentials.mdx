...
| Zalo | SocialAPI | Click Connect | Run OAuth, manage tokens | No |
...
| Bluesky | **You (app password)** | Create an app password in Bluesky settings, paste it into SocialAPI | Resolve your handle, validate the password, manage the session | No (no app review, no OAuth) |
...
Telegram does not use OAuth at all. Instead of clicking Connect and authorizing through a consent screen, you create a bot with [@BotFather](https://t.me/BotFather) (send `/newbot`, choose a name and a username ending in `bot`), copy the token it gives you, and paste it when connecting. The token never expires and is revoked only through @BotFather. There is no developer account, business verification, or app review. See the [Telegram connector](/connectors/telegram) for the full flow.

## Bluesky uses an app password

Bluesky is the other connector with no OAuth flow. You create an app password in Bluesky under **Settings > App passwords**, then paste it with your handle when connecting. App passwords are separate from your account password and can be revoked individually, so never paste your real account password.

Tick **Allow access to your direct messages** when you create it if you want SocialAPI to read and send DMs. That option can only be set at creation time and cannot be added to an existing app password. See the [Bluesky connector](/connectors/bluesky) for the full flow.
