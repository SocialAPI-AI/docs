...
| Like comment | Conditional | Accounts connected with Facebook Login only. Instagram Login accounts return `501 resource.not_supported`: read `can_like` on each comment rather than assuming. Instagram does not report who liked a comment, so `liked_by_viewer` reflects likes made through SocialAPI and not likes made in the Instagram app. |
...
- **Private replies.** Replying to a comment with `"private": true` sends a DM to the commenter. This is Instagram's native "Send Private Reply" feature, not a [SocialAPI](https://social-api.ai) workaround. Meta allows one private reply per comment, within 7 days of the comment, and the user must respond before you can send another message. The private-reply endpoint also accepts `buttons` (1 to 3), sent as a button template, which gives the commenter something to tap to continue the conversation.
