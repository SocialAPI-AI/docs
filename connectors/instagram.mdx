...
Pass these inside `platform_data.instagram` when creating a post.

| Field | Type | Description |
|---|---|---|
| `alt_text` | string | Alt text for images (accessibility). Applied to the image container. |
| `location_id` | string | Facebook location page ID to tag the post with a location. |
| `share_to_feed` | bool | For Reels, whether to also share to the main profile feed. |
| `cover_url` | string | Custom cover image URL for video or Reel posts. |

These fields are passed directly as parameters to the Instagram Graph API container creation endpoint. Any additional key-value pairs in `platform_data.instagram` are forwarded as-is.
