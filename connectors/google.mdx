...
To check where a reply ended up, read the review again with `GET /v1/inbox/reviews/{account_id}` and look at `reply.state`. A rejected reply also carries `reply.policy_violation`, passed through exactly as Google sends it:
