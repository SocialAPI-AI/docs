...
| Comments | Yes | Q&A questions on your Business Profile (location-wide, not per post) |
| Reply to comment | Yes | Replies to Q&A questions |
| Comment replies (thread) | Yes | Q&A answer threads |
...
- **Review ratings** are returned in `metadata.star_rating` as a string (e.g. `"ONE"`, `"TWO"`, `"THREE"`, `"FOUR"`, `"FIVE"`).
