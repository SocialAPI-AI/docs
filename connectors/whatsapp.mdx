...
WhatsApp has no message-history API, so the inbox covers messages from connect time onward. There is no backfill of messages sent before you connected.
...
WhatsApp exposes inbound media only to the connected account, never to an external caller, so it cannot be linked directly. For WhatsApp, and only WhatsApp, `messages.attachment_url` is a SocialAPI URL (`/v1/inbox/media/{id}`) that you retrieve with your API key, unlike every other connector where `attachment_url` is the platform's own URL. See [Inbox media](/guides/inbox#media) for the endpoint and its response codes.
