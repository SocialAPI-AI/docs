...
| Template messages | No | Templates are created and managed in Meta Business Manager, but sending them is not exposed through our API yet |
...
- **Outside the window:** WhatsApp requires an approved message template. Templates are created and managed in Meta Business Manager. Sending templates is **not yet available through our API**, so a reply attempted after the window has closed returns `whatsapp.window_closed` (see [Errors](#errors)).
