...
Zalo Official Accounts publish articles (and video posts) rather than short-form posts. Article creation is **asynchronous**: after you submit a draft, the target settles once Zalo finishes processing the article and returns its final ID. See [Zalo publishing](/posts/zalo) for the full request and response shapes, including the `title` requirement enforced before an article can be submitted.
