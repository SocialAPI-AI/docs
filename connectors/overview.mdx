...
[SocialAPI.AI](https://social-api.ai) uses a connector pattern - each platform implements the same interface. If a platform doesn't support a feature, the API returns `501` with `code: "not_supported"`. Your code doesn't need to branch per platform; just handle the 501.
