...
Pass these inside `platform_data.facebook` when creating a text post.

| Field | Type | Description |
|---|---|---|
| `link` | string | URL to attach as a link preview in the post. |
| `picture` | string | Image URL to use as the link preview thumbnail. |

These fields are passed directly as query parameters to the Facebook Graph API feed endpoint. Any additional string key-value pairs in `platform_data.facebook` are forwarded as-is.

<Note>
PlatformData fields only apply to text posts. Photo posts use `media_ids` for the image URL and do not pass through platform data.
</Note>
