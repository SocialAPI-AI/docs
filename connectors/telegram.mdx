...
Telegram has no message-history API: a bot only sees messages that arrive after it is connected. The inbox covers messages received from connect time onward.
...
Use the [`attachments` array](/guides/inbox#attachments) to send files. SocialAPI picks the Telegram method based on the URL extension: image extensions (`.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`) call `sendPhoto`; all other extensions call `sendDocument`. Telegram supports a `caption` per attachment.
...
curl -X POST https://api.social-api.ai/v1/inbox/conversations/{conversation_id}/messages \
  ...
  -d '{
    "account_id": "{account_id}",
    "attachments": [
      { "source_type": "url", "source": "https://example.com/report.pdf", "caption": "Here is the file you requested." }
    ]
  }'
```

The single-attachment shorthand `attachment_url` still works, and with it the message `text` is used as the caption.
...
- **No message history backfill.** There is no list or history endpoint in the Bot API. The inbox covers messages received after you connect.
