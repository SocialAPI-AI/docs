...
Use the `id` from the comment above. The `sapi_cmt_` prefix tells the API this is a comment — no extra parameters needed.

```bash
curl -X POST \
  "https://api.social-api.ai/v1/accounts/acc_01HZ9X3Q4R5M6N7P8V2K0W1J/interactions/sapi_cmt_aW5zdGFncmFtOjE3ODQxNDA1/reply" \
  -H "Authorization: Bearer $SOCAPI_KEY" \
