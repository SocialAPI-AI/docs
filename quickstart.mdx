...
Text-only posts need just the call above. To attach an image, upload it first, verify it, then reference it in the `media` array. This is a four-step flow:
...
Each `media` item names its `source_type`. Here it is `media_id`, referencing the upload above. You can skip steps 1 to 3 entirely by pointing at a public URL instead, with `{ "source_type": "url", "source": "https://cdn.example.com/launch.jpg" }`. Skipping step 3 on the upload path leaves the media `pending`, and the publish fails rather than posting without it. See [Media uploads](/guides/media) for URLs, platform attachment IDs, videos, and carousels.

The older flat `media_ids: ["..."]` array still works and is equivalent to a list of `media_id` sources.
